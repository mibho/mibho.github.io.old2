...
This might be just what we were looking for. Unfortunately, it's possible there may be more in the sections we haven't examined yet, so for now, we'll put this aside and come back to it later. (On the bright side, we'll be zooming past the remaining ones since we're just checking for any other modifications to the file data, if any).

Let's name it (*sub_4011F0*) **maybeDecrypt** and return back to *loc_4012AD*.  

![image](https://user-images.githubusercontent.com/86342821/144698605-6d3ac5c3-965b-4f70-b020-a2d6060b333b.png)

All that happens (after we return from **maybeDecrypt**) is a call to *WriteFile*:

```
WriteFile(hObject, &Buffer, NumberOfBytesRead, &NumberOfBytesWritten, 0)
```

and then we take the jump to *loc_401282*:

![image](https://user-images.githubusercontent.com/86342821/144698765-3a260811-c33f-448e-b56b-c1c8af5fe604.png)

which means this is a loop. Let's consider the fail branch this time:

![image](https://user-images.githubusercontent.com/86342821/144698956-dc0cc617-8d75-4a48-a36c-e54dadf26cad.png)

This takes us to *loc_4012E4*:

![image](https://user-images.githubusercontent.com/86342821/144699129-397c2230-8a2b-4567-a4af-42c4b765105c.png)

3 different functions are called here: *CloseHandle*, *WriteConsoleA*, and *sub_401000*. Thankfully, none of the arguments pertain to file data or the user input buffer.

![image](https://user-images.githubusercontent.com/86342821/144699513-ff55a7f5-4e82-494a-b977-700afcd9cb6d.png)

Returning brings us back to *loc_4013BE* in  **doesSomethingAndRetExitCode**. The function we just came from was *sub_401220* which I'll refer to as **readAndWriteDataFile**.
